...
class TestRAGEngine:
    ...
    def rag_engine(self):
        ...
        with patch("src.rag_engine.OpenAIEmbeddings"), patch("src.rag_engine.ChatOpenAI"):
            return RAGEngine()
    ...
    @patch("src.rag_engine.Chroma")
    def test_index_transcripts_success(self, mock_chroma, rag_engine, sample_transcripts):
    ...
    def test_chat_without_chain(self, rag_engine):
        ...
        with patch.object(rag_engine, "setup_conversation_chain"):
            with patch.object(
                rag_engine,
                "conversation_chain",
                MagicMock(return_value={"answer": "Test answer", "source_documents": []}),
            ):
